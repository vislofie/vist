...
public:
    ...
    void async_accept();
    void post_from(std::shared_ptr<client_session> from, const std::string& message) const;
    void post_all(const std::string& message) const;

private:
    bool is_user_logged_in(std::string_view username) const;
    void process_client_message(std::shared_ptr<client_session> client, std::string& msg) const;
